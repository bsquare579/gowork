    ...
    <script src="https://maps.googleapis.com/maps/api/js?libraries=places&callback=initAutocomplete&language=eng&output=json&key={{ env('API_KEY') }}&callback=initAutocomplete" async defer></script>
        ...
        <form action="{{ url('/company', $row->id) }}" method="POST">
          ...
          <div class="form-group">
            ...
            <input type="text" class="form-control col-lg-9" id="name" placeholder="Enter Bussiness Name" name="name" value="{{$row->name}}" required readonly>
          </div>
          <div class="form-group">
            <label for="email">Email</label>
            <input type="email" class="form-control col-lg-9" id="email" placeholder="Enter Your Email" name="email" value="{{$row->email}}" required readonly>
          </div>
          <div class="form-group">
          ...
          <br/>
          <button type="submit" class="btn btn-primary">Submit</button>&ensp;<a href="/user/company/create"><button type="button" class="btn btn-primary"> Register New Bussiness</button></a>
        </form>
        @endforeach
  </div>
</div>
